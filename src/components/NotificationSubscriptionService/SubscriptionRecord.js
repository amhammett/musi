...
class JsonRecord extends React.Component {
  ...
  componentDidMount() {
    ...
    const url = `${Settings.endpoints.NSS_ENDPOINT}/${this.props.id}`
    fetch(url)
    .then((response) => {
...
const SubscriptionRecord = ({match}) => (
  <div className="page-view page-master">
    <h2>Notification Subscription Record</h2>
    <p>
      Subscriptions for notifications based on job pathing
    </p>
    <Paper className="page" zDepth={1}>
      <Toolbar>
        <ToolbarGroup>
          <ToolbarTitle text="Subscription Record" />
        </ToolbarGroup>
      </Toolbar>
